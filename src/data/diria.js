const diria = [
  {
    slug: "claves-7",
    title: "Claves compuestas en 7/8",
    content: {
      sections: [
        {
          type: "div",
          content: `<p>Para la creación de los ritmos en 7, al igual que en los ritmos en 5, fue muy importante el desarrollo de distintas claves compuestas que le dieran sentido y cohesión a estos ritmos; para ello se crearon 4 claves compuestas, cada una con una duración de dos compases en 7/8.</p>`,
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "claves-7",
          slides: [],
          playlists: [
            {
              id: "claves-7",
              data: [
                {
                  src: "../assets/audio/convite-7/00-claves-7/00-click-120bpm.mp3",
                  name: "Click 120bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/00-claves-7/01-clave-completa-son.mp3",
                  name: "Clave Completa en 7/8",
                  customClass: "setima-clave",
                },
                {
                  src: "../assets/audio/convite-7/00-claves-7/02-clave-abierta-son.mp3",
                  name: "Clave Abierta en 7/8",
                  muted: true,
                  customClass: "setima-cata",
                },
                {
                  src: "../assets/audio/convite-7/00-claves-7/03-clave-son_3-4.mp3",
                  name: "Clave 3 - 4 en 7/8",
                  muted: true,
                  customClass: "setima-chequere",
                },
                {
                  src: "../assets/audio/convite-7/00-claves-7/04-clave-son_4-3.mp3",
                  name: "Clave 4 - 3 en 7/8",
                  muted: true,
                  customClass: "variacion-setima-chequere",
                },
              ],
            },
          ],
        },
      ],
    },
  },
  {
    slug: "completa-7",
    title: "Clave Completa en 7",
    content: {
      sections: [
        {
          type: "text",
          content:
            `Este ritmo se creó a partir de la clave compuesta que lleva el mismo nombre (“Clave Completa en 7”), la cual se deriva de la “Clave Completa” en 5 utilizando la lógica de la Hemiola. El Chequeré presente en este reproductor corresponde al Chequeré Variación de la partitura en el libro.`,
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "clave-completa",
          slides: [],
          playlists: [
            {
              id: "clave-completa",
              data: [
                {
                  src: "../assets/audio/convite-7/01-clave-completa/00-click-175bpm.mp3",
                  name: "Click 175bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/01-clave-completa/01-clave.mp3",
                  name: "Claves",
                  customClass: "setima-clave",
                },
                {
                  src: "../assets/audio/convite-7/01-clave-completa/02-cata.mp3",
                  name: "Catá",
                  customClass: "setima-cata",
                },
                {
                  src: "../assets/audio/convite-7/01-clave-completa/03-chequere.mp3",
                  name: "Chequeré Variación",
                  customClass: "setima-chequere",
                },
                {
                  src: "../assets/audio/convite-7/01-clave-completa/04-quinto.mp3",
                  name: "Quinto",
                  customClass: "setima-quinto",
                },
                {
                  src: "../assets/audio/convite-7/01-clave-completa/05-conga.mp3",
                  name: "Conga",
                  customClass: "setima-completa-conga",
                },
                {
                  src: "../assets/audio/convite-7/01-clave-completa/06-tumba.mp3",
                  name: "Tumba",
                  customClass: "setima-tumbadora",
                },
                {
                  src: "../assets/audio/convite-7/01-clave-completa/07-campana.mp3",
                  name: "Campana",
                  muted: true,
                  customClass: "setima-campana",
                },
                {
                  src: "../assets/audio/convite-7/01-clave-completa/08-campana_mano.mp3",
                  name: "Campana de mano",
                  muted: true,
                  customClass: "variacion-setima-campana",
                },
              ],
            },
          ],
        },
        {
          type: "header",
          content: "Clave completa en 7 - variante rápida",
        },
        {
          type: "text",
          content:
            `La pista usada en este reproductor para el Chequeré corresponde al Chequeré de la partitura en el libro. Además en este reproductor no se incluye la pista correspondiente al Quinto de la partitura en el libro.`,
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "clave-completa-rapida",
          slides: [],
          playlists: [
            {
              id: "clave-completa-rapida",
              data: [
                {
                  src: "../assets/audio/convite-7/02-clave-completa_rapida/00-click-203bpm.mp3",
                  name: "Click 203bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/02-clave-completa_rapida/01-clave.mp3",
                  name: "Clave",
                  customClass: "setima-clave",
                },
                {
                  src: "../assets/audio/convite-7/02-clave-completa_rapida/02-cata.mp3",
                  name: "Catá",
                  customClass: "setima-cata",
                },
                {
                  src: "../assets/audio/convite-7/02-clave-completa_rapida/03-chequere.mp3",
                  name: "Chequeré",
                  customClass: "setima-chequere",
                },
                {
                  src: "../assets/audio/convite-7/02-clave-completa_rapida/04-conga.mp3",
                  name: "Conga",
                  customClass: "setima-completa-conga",
                },
                {
                  src: "../assets/audio/convite-7/02-clave-completa_rapida/05-tumba.mp3",
                  name: "Tumba",
                  customClass: "setima-tumbadora",
                },
                {
                  src: "../assets/audio/convite-7/02-clave-completa_rapida/06-campana.mp3",
                  name: "Campana",
                  muted: true,
                  customClass: "setima-campana",
                },
                {
                  src: "../assets/audio/convite-7/02-clave-completa_rapida/07-campana_mano.mp3",
                  name: "Campana de mano",
                  muted: true,
                  customClass: "variacion-setima-campana",
                },
              ],
            },
          ],
        },
      ],
    },
  },
  {
    slug: "abierta-7",
    title: "Clave Abierta en 7",
    content: {
      sections: [
        {
          type: "text",
          content:
            "Este ritmo se creó a partir de la clave compuesta que lleva el mismo nombre (“Clave Abierta en 7”), la cual se deriva de la “Clave Abierta” en 5, utilizando la lógica de la Hemiola.",
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "clave-abierta",
          slides: [],
          playlists: [
            {
              id: "clave-abierta",
              data: [
                {
                  src: "../assets/audio/convite-7/03-clave-abierta/00-click-238bpm.mp3",
                  name: "Click 238bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/03-clave-abierta/01-clave.mp3",
                  name: "Clave",
                  customClass: "setima-abierta-clave",
                },
                {
                  src: "../assets/audio/convite-7/03-clave-abierta/02-chequere.mp3",
                  name: "Chequeré",
                  customClass: "setima-abierta-chequere",
                },
                {
                  src: "../assets/audio/convite-7/03-clave-abierta/03-chequere_golpe.mp3",
                  name: "Chequeré Variación",
                  muted: true,
                  customClass: "variacion-setima-abierta-chequere",
                },
                {
                  src: "../assets/audio/convite-7/03-clave-abierta/04-conga.mp3",
                  name: "Conga",
                  customClass: "setima-abierta-conga",
                },
                {
                  src: "../assets/audio/convite-7/03-clave-abierta/05-tumba.mp3",
                  name: "Tumbadora",
                  customClass: "setima-abierta-tumbadora",
                },
                {
                  src: "../assets/audio/convite-7/03-clave-abierta/06-tumba_variacion.mp3",
                  name: "Tumbadora Variación",
                  muted: true,
                  customClass: "variacion-setima-abierta-tumbadora",
                },
                {
                  src: "../assets/audio/convite-7/03-clave-abierta/07-campana_mano.mp3",
                  name: "Campana de mano",
                  muted: true,
                  customClass: "setima-abierta-campana-mano",
                },
              ],
            },
          ],
        },
      ],
    },
  },
  {
    slug: "septima",
    title: "Séptima",
    content: {
      sections: [
        {
          type: "text",
          content:
            "Este ritmo se creó tomando como base el patrón del tambor Conga del Son Cubano, el cual también se utiliza en una de las variantes del Calypso limonense (Costa Rica) (Saavedra Reyes, C. 1999). En esta ocasión se mantiene la misma estructura del patrón, pero se elimina el golpe que corresponde a la corchea del contratiempo del segundo tiempo de cada compás (el cuarto golpe del tambor Conga, la cuarta corchea), quedando así la figura completa dentro de un compás de 7/8.",
        },
        {
          type: "header",
          content: "Séptima 4 - 3 en 7",
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "septima-4-3",
          slides: [],
          playlists: [
            {
              id: "septima-4-3",
              data: [
                {
                  src: "../assets/audio/convite-7/04-septima/00-click-120bpm.mp3",
                  name: "Click 120bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/01-clave-son_4-3.mp3",
                  name: "Clave 4 - 3",
                  customClass: "setima-clave-4-3",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/05-conga-son.mp3",
                  name: "Conga",
                  customClass: "setima-conga",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/07-timbales_4-3.mp3",
                  name: "Timbales",
                  customClass: "setima-timbal",
                },
              ],
            },
          ],
        },
        {
          type: "header",
          content: "Séptima 3 - 4 en 7",
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "septima-3-4",
          slides: [],
          playlists: [
            {
              id: "septima-3-4",
              data: [
                {
                  src: "../assets/audio/convite-7/04-septima/00-click-120bpm.mp3",
                  name: "Click 120bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/02-clave-son_3-4.mp3",
                  name: "Clave 3 - 4",
                  customClass: "setima-clave-3-4",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/05-conga-son.mp3",
                  name: "Conga",
                  customClass: "setima-conga",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/06-timbales_3-4.mp3",
                  name: "Timbales",
                  customClass: "setima-timbal",
                },
              ],
            },
          ],
        },
        {
          type: "header",
          content: "Séptima Clave Completa en 7",
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "septima-clave-completa",
          slides: [],
          playlists: [
            {
              id: "septima-clave-completa",
              data: [
                {
                  src: "../assets/audio/convite-7/04-septima/00-click-120bpm.mp3",
                  name: "Click 120bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/03-clave-completa-son.mp3",
                  name: "Clave Completa en 7",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/05-conga-son.mp3",
                  name: "Conga",
                  customClass: "setima-conga",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/07-timbales_4-3.mp3",
                  name: "Timbales",
                  customClass: "setima-timbal",
                },
              ],
            },
          ],
        },
        {
          type: "header",
          content: "Séptima Clave Abierta en 7",
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "septima-clave-abierta",
          slides: [],
          playlists: [
            {
              id: "septima-clave-abierta",
              data: [
                {
                  src: "../assets/audio/convite-7/04-septima/00-click-120bpm.mp3",
                  name: "Click 120bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/04-clave-abierta-son.mp3",
                  name: "Clave Abierta en 7",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/05-conga-son.mp3",
                  name: "Conga",
                  customClass: "setima-conga",
                },
                {
                  src: "../assets/audio/convite-7/04-septima/06-timbales_3-4.mp3",
                  name: "Timbales",
                  customClass: "setima-timbal",
                },
              ],
            },
          ],
        },
      ],
    },
  },
  {
    slug: "septimo",
    title: "Séptimo",
    content: {
      sections: [
        {
          type: "div",
          content:
            `<p>Este ritmo se creó tomando como base el patrón del tambor Conga del Son Cubano, el cual también se utiliza en una de las variantes del Calypso limonense (Costa Rica) (Saavedra Reyes, C. 1999).</p>
            <p>En la versión 4 - 3, se elimina el segundo golpe del tambor Conga (segunda corchea de cada compás) y el primer golpe abierto del patrón se adelanta una corchea, quedando en el quinto tiempo del compás. En esta versión se puede utilizar tanto la clave compuesta de la forma 4 - 3 en 7, como la “Clave Completa en 7".</p>
            <p>En la versión 3 - 4, se elimina el segundo golpe del tambor Conga (segunda corchea de cada compás), el resto del patrón se mantiene igual, quedando así la figura completa dentro de un compás de 7/8 . Para este patrón es conveniente utilizar un manoteo alternado (izquierda - derecha - izquierda), parecido al que se utiliza en una de las variantes del Calypso de Limón que tiene sus raíces en el Calypso trinitario (Saavedra Reyes, C. 1999). En esta versión se puede utilizar tanto la clave compuesta de la forma 3 - 4 en 7, como la “Clave Abierta en 7" (aunque la “Clave Abierta en 7” está en un compás de amalgama de 2 - 4 - 2).</p>
            `,
        },
        {
          type: "header",
          content: "Séptimo 4 - 3",
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "septimo-4-3",
          slides: [],
          playlists: [
            {
              id: "septimo-4-3",
              data: [
                {
                  src: "../assets/audio/convite-7/05-septimo/00-click-120bpm.mp3",
                  name: "Click 120bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/05-septimo/02-clave_4-3.mp3",
                  name: "Clave 4 - 3",
                  customClass: "setima-clave-4-3",
                },
                {
                  src: "../assets/audio/convite-7/05-septimo/06-conga_pandeiro-2.mp3",
                  name: "Conga Variación",
                  customClass: "setima-conga",
                },
              ],
            },
          ],
        },
        {
          type: "header",
          content: "Séptimo 3 - 4",
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "septimo-3-4",
          slides: [],
          playlists: [
            {
              id: "septimo-3-4",
              data: [
                {
                  src: "../assets/audio/convite-7/05-septimo/00-click-120bpm.mp3",
                  name: "Click 120bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/05-septimo/01-clave_3-4.mp3",
                  name: "Clave 3 - 4",
                  customClass: "setima-clave-3-4",
                },
                {
                  src: "../assets/audio/convite-7/05-septimo/05-conga_pandeiro.mp3",
                  name: "Conga",
                  customClass: "setima-conga",
                },
              ],
            },
          ],
        },
        {
          type: "header",
          content: "Séptimo de clave completa en 7",
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "septimo-clave-completa",
          slides: [],
          playlists: [
            {
              id: "septimo-clave-completa",
              data: [
                {
                  src: "../assets/audio/convite-7/05-septimo/00-click-120bpm.mp3",
                  name: "Click 120bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/05-septimo/04-clave_completa.mp3",
                  name: "Clave Completa en 7",
                  customClass: "setima-clave-3-4",
                },
                {
                  src: "../assets/audio/convite-7/05-septimo/06-conga_pandeiro-2.mp3",
                  name: "Conga Variación",
                  customClass: "setima-conga",
                },
              ],
            },
          ],
        },
        {
          type: "header",
          content: "Séptimo de Clave Abierta en 7",
        },
        {
          type: "audio",
          content: "audio",
          idPlayer: "septimo-clave-abierta",
          slides: [],
          playlists: [
            {
              id: "septimo-clave-abierta",
              data: [
                {
                  src: "../assets/audio/convite-7/05-septimo/00-click-120bpm.mp3",
                  name: "Click 120bpm",
                  muted: true,
                  customClass: "click",
                },
                {
                  src: "../assets/audio/convite-7/05-septimo/03-clave_abierta.mp3",
                  name: "Clave Abierta en 7",
                  customClass: "setima-clave-3-4",
                },
                {
                  src: "../assets/audio/convite-7/05-septimo/05-conga_pandeiro.mp3",
                  name: "Conga",
                  customClass: "setima-conga",
                },
              ],
            },
          ],
        },
      ],
    },
  }
];

export default diria;